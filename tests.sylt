...
let prelude_lib() = {
	## tostring
		...
		= "[\"bananas\", 18, true]")
	ensure(tostring(tostring) = "tostring")
...
let math_lib() = {
	...
	## euler's number
	ensure(nearly(E, 2.718281, 0.001))
	...
	let log_raise(base, target) = {
		let power = log(target, base)
		let result = raise(base, power)
		ensure(nearly(result, target, 0.001))
	}
	
	foreach(bases, fun(_, base) ->
		log_raise(base, target))
